...
export function Franchise({ mode = 'teaser' }: { mode?: 'teaser' | 'page' }) {
  ...
  const franchiseMailto = `mailto:${SITE.franchiseEmail}?subject=${encodeURIComponent(
    isES ? 'Quiero información sobre franquicias Fumigaciones Hernández' : 'Fumigaciones Hernández Franchise Inquiry'
  )}`

  return (
    <section
      id="franquicias"
      className="section-padding relative overflow-hidden bg-gradient-to-br from-[#B39B00] to-[#F07070] text-white"
    >
      {/* Franja tricolor decorativa superior */}
      <div className="absolute inset-x-0 top-0 h-1 flex">
        <span className="flex-1 bg-[#F07070]" />
        <span className="flex-1 bg-white" />
        <span className="flex-1 bg-[#FFDF00]" />
      </div>
          ...
          <FadeUp>
            <div className="space-y-6 max-w-xl">
              ...
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter uppercase leading-[1.1]">
                ...
                <span className="text-[#FFF8DC]">Fumigaciones Hernández</span>
              </h2>

              <div className="w-24 h-1 bg-[#FFDF00]" />

              <p className="text-white/90 text-[1rem] md:text-[1.125rem] font-medium leading-relaxed">
                {isES
                  ? 'Súmate a la marca líder en control de plagas de México. Un modelo de negocio rentable, con respaldo total y demanda todo el año. Lleva Fumigaciones Hernández a tu ciudad.'
                  : 'Join Mexico’s leading pest control brand. A profitable business model with full support and year-round demand. Bring Fumigaciones Hernández to your city.'}
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 pt-2">
                {mode === 'page' ? (
                  <a
                    href="#franchise-form"
                    className="inline-flex items-center justify-center gap-2.5 bg-[#FFDF00] hover:bg-[#D9BE00] text-white font-black text-sm px-8 py-4 rounded shadow-lg transition-all duration-200 uppercase tracking-wider group"
                  >
          ...
          <FadeUp delay={0.15}>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {benefits.map(({ icon: Icon, title, desc }, i) => (
                ...
                >
                  <div className="w-11 h-11 rounded bg-[#FFDF00] flex items-center justify-center mb-4">
